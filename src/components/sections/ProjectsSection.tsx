...
const ProjectsSection = () => {
  ...
  return (
    <>
      <div className="max-w-[1200px] mx-auto px-5 md:px-14 py-16 md:py-24" id="projects">
        <SectionHeader
          number="06"
          label="Portfolio"
          title="Projects &"
          titleAccent="Experience"
          description="Academic, innovation and research projects."
          chip={`${projects.length} Projects`}
        />

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:flex md:overflow-x-auto md:grid-cols-none scrollbar-hide md:pb-3">
          {projects.map((project) => (
            <div key={project.title} className="md:flex-shrink-0 md:w-[320px] lg:w-auto lg:flex-1">
              <ProjectCard project={project} onViewPdfs={setPdfProject} onViewDetails={setDetailsProject} />
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center mt-8">
          <button
            ...
            className="px-7 py-3 rounded-full bg-gradient-to-br from-p1 to-p2 text-primary-foreground font-bold text-[0.82rem] tracking-wide shadow-[0_4px_14px_rgba(99,102,241,0.3)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_28px_rgba(99,102,241,0.4)] active:scale-[0.97]"
          >
            View All Projects →
          </button>
        </div>
